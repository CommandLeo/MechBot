...
const pollOptions = Array.from({ length: 20 }, (_, i) => ({ type: 'STRING', name: `option-${i + 1}`, description: `Option ${i + 1} for the poll` }));
...
export const command = {
	...
	async execute(interaction) {
		...
		if (subCommand == 'create') {
			...
			const voteButton = new MessageButton({ customId: `poll-vote-${pollId}`, label: 'Vote', emoji: '🗳', style: 'PRIMARY' });
			const retractButton = new MessageButton({ customId: `poll-retract-${pollId}`, label: 'Retract', style: 'DANGER' });
			const row = new MessageActionRow({ components: [voteButton, retractButton] });

			await interaction.editReply({ embeds: [embed], components: [row] });

			const Polls = interaction.client.sequelize.model('polls');
			...
			const resultsEmbed = new MessageEmbed({ color: '#3498db', title: 'Poll Results', image: { url: 'attachment://poll-results.png' } });
