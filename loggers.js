...
export function logDM(message) {
	const BOT_SPAM_CHANNEL = message.client.CHANNELS.BOT_SPAM;
...
export function logMessageDeleted(message, executor, reason) {
	const LOG_CHANNEL = message.client.CHANNELS.LOG;
...
export function logRoleGiven(giver, target, role, reason) {
	const LOG_CHANNEL = role.client.CHANNELS.LOG;
...
export function logMemePrisoner(member) {
	const MEME_PRISONER_ROLE = member.client.ROLES.MEME_PRISONER;
	const LOG_CHANNEL = member.client.CHANNELS.LOG;
...
export function logBan(user, executor, reason) {
	const LOG_CHANNEL = user.client.CHANNELS.LOG;
